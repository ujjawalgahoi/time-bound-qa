...
function QuestionBoard(props) {
	...
	const handlePreviousBtn = () => {
		...
		if(tempOptions[props.currentIndex] === -2) tempOptions[props.currentIndex] = -1
      ...
      <div className={styles["option-container"]}>
        {props.stages[props.currentIndex].options.map((option, index) => {
          return (
			...
			{props.currentIndex > 0 && <button className={styles['prev']} onClick={handlePreviousBtn}>Previous</button>}
			{(props.currentIndex < props.stages.length - 1) && <button className={styles['next']} onClick={handleNextBtn}>Next</button>}
			</div>
    </div>
  );
}
